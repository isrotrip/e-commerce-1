<template>
  <v-dialog v-model="dialog" persistent max-width="600px">
    <template v-slot:activator="{ on }">
      <v-btn color="green" flat v-on="on">Update a Cart</v-btn>
    </template>
    <v-card>
      <v-card-title>
        <span class="headline">Update a Cart</span>
      </v-card-title>
      <v-card-text>
      ...
      <v-card-actions>
        ...
        <v-btn color="blue darken-1" flat @click="updateCart">Update</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    updateCart () {
      serverAPI.put(`/carts/${this.cart._id}`,
        {
          amount: this.amount,
